...
"""
    if avg_format < 0.95:
        report += "- ⚠️ Format compliance below 95% — check whether the model output format changed\n"
    if actionability < 0.20:
        report += "- ⚠️ Actionability below 20% — reviews may be too generic or ignored\n"
    if avg_score and avg_score > 8.5:
        report += "- ⚠️ Average score > 8.5 — AI may be too lenient (score bias)\n"
    if avg_score and avg_score < 5.0:
        report += "- ⚠️ Average score < 5.0 — AI may be too harsh\n"
    if not any([avg_format < 0.95, actionability < 0.20, avg_score and (avg_score > 8.5 or avg_score < 5.0)]):
        report += "- ✅ All metrics within range\n"
