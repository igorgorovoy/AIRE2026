...
abox is a **local AI infrastructure sandbox**. A single `make run` bootstraps Flux into an existing local Kubernetes cluster (default context: Rancher Desktop) and reconciles a full AI stack via Flux GitOps. There is no application code — the repo is entirely Kubernetes manifests, OpenTofu, and shell scripts.
...
| Layer | Tech | Version |
|---|---|---|
| Cluster | Rancher Desktop Kubernetes | latest |
...
A single `tofu apply` bootstraps Flux on the active Kubernetes context:

```
Kubernetes context (rancher-desktop by default)
...
```
bootstrap/           OpenTofu: flux.tf, providers.tf, variables.tf
releases/
